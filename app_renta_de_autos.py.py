...
APP_VERSION = "QuitoDrive v7.0 · Reservaciones y disponibilidad"
    ...
    if selected_car:
        ...
        with selected_details:
            with st.container(border=True):
                ...
                if st.button(
                    "Ir a disponibilidad y reservación",
        ...
        availability_preview = availability_rows(
            selected_car["id"],
            30,
        )
        available_days = sum(
        ...
        availability_metric_1, availability_metric_2 = st.columns(2)
        availability_metric_1.metric(
            "Disponibilidad hoy",
            (
                "Disponible"
                if is_available(
        ...
        availability_metric_2.metric(
            "Días libres próximos 30",
            f"{available_days} de 30",
        )
        ...
        with st.expander(
            ...
            periods = reserved_periods(
                selected_car["id"]
            )

            if periods:
                st.write("**Periodos ya reservados:**")
                for period in periods:
                    st.warning(
                        f"{period['fecha_inicio']} al "
                        f"{period['fecha_fin']}"
                    )
            else:
                st.success(
                    "Este vehículo no tiene reservaciones futuras."
                )
...
"""
    ...
    if selected_car:
        with st.container(border=True):
            reservation_image, reservation_details = st.columns(
                [1, 1.25]
            )
            ...
            reservation_availability = availability_rows(
                selected_car["id"],
                45,
            )
            ...
            free_days = sum(
                1
                for row in reservation_availability
                if row["Estado"] == "Disponible"
            )

            availability_col_1, availability_col_2 = st.columns(2)
            availability_col_1.metric(
                "Estado de hoy",
                (
                    "Disponible"
                    if is_available(
            ...
            availability_col_2.metric(
                "Días disponibles",
                f"{free_days} de 45",
            )

            with st.expander(
                "Calendario de disponibilidad de los próximos 45 días",
                expanded=True,
            ):
                st.dataframe(
                ...
                occupied_periods = reserved_periods(
                    selected_car["id"]
                )

                if occupied_periods:
                    st.write("**Fechas no disponibles:**")
                    for period in occupied_periods:
                        st.warning(
                            f"Del {period['fecha_inicio']} "
                            f"al {period['fecha_fin']}"
                        )
                else:
                    st.success(
                        "No existen reservas futuras para este vehículo."
                    )

            st.subheader("Seleccionar fechas")
            ...
            if available:
                st.success(
                    "El vehículo está disponible durante "
                    "todo el periodo seleccionado."
                )
            else:
                st.error(
                    "Una o más fechas seleccionadas ya están reservadas. "
                    "Escoge otro periodo."
                )
            ...
            if not st.session_state.user:
                st.warning(
                    "Para guardar la reservación debes iniciar sesión "
                    "o crear una cuenta en la sección 02."
                )

            if st.button(
                "Confirmar y guardar reservación",
                key=(
                    f"confirm_reservation_"
                    f"{selected_car['id']}"
                ),
                type="primary",
        ...
        st.info(
            "Selecciona primero un vehículo en el catálogo. "
            "Después aparecerán aquí su disponibilidad, "
            "el calendario y el botón para reservar."
        )
    ...
    if st.session_state.user:
        if st.session_state.last_code:
            ...
            st.caption(
                "Guarda este código para consultar la reservación."
            )
